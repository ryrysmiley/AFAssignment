...
export function GenerateCadets(afscs) {
	...
	for (let i = 0; i < numCadets; ++i) {
		...
		cadetList.push(
			new Cadet(`Cadet ${i + 1}`, preferences, percentile, usafaCadet, degrees)
		);
	}
	return cadetList;
}
...
function generateDegrees(preferences) {
	...
	const potentialDegrees = [
		DegreeEnum.MANDATORY,
		DegreeEnum.DESIRED,
		DegreeEnum.PERMITTED,
	];
...
function weightedRandomChoice(cumulativeWeights) {
	const totalWeight =
		cumulativeWeights[cumulativeWeights.length - 1].cumulativeWeight;
