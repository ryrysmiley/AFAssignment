...
export default function AfscEditor() {
  const [afscs, setAfscs] = useState([]);
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 10;  // You can change this to adjust how many rows per page.
  ...
  function addAFSC() {
    const newAfsc = {
      ...
      target: 0,
      overclassFactor: 0,
      mandatoryDegreeBounds: { min: 0, max: 0 },
      usaCadetBounds: { min: 0, max: 0 },
      meritBounds: { min: 0, max: 0 },
    ...
    <div className="p-6">
      ...
      <div className="flex justify-between items-center mb-4">
        <button
          ...
          className="p-2 bg-gray-300 text-gray-700 rounded-md disabled:cursor-not-allowed"
        >
          Previous
        </button>
