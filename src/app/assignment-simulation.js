...
export function AssignmentSimulation({ afscs, cadets }) {
	...
	useEffect(() => {
		...
		const sortedPreferences = Object.entries(prefCount).sort(
			(a, b) => b[1] - a[1]
		); // Sort based on the second element (count), descending
	...
	async function RunSimulation() {
		...
		const simulation = new CadetAssignment(
			cadetData,
			afscData,
			deviationPenalty,
			glpkInstance
	...
	return (
		<div className="p-6">
			...
			<p className="mb-4 text-gray-600">
				Simulate air force specialty code job assignment and view job and cadet
				properties.
			</p>
				...
				<div className="overflow-x-auto mb-4">
					<table className="border-collapse border border-gray-300 me-8">
						...
						<tbody>
							...
							{cadets.length > 0 &&
								Object.entries(preferenceCount).map(([afsc, count], index) => (
									<tr
										key={index}
										className={`${
							...
							{Object.keys(results).length > 0 && (
								<tr>
									{Object.entries(results).map(([pref, count]) => (
										<td className="px-4 py-2" key={pref}>
											{count}
										</td>
									))}
								</tr>
